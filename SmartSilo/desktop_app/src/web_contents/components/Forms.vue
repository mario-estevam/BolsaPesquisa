<template>
  ...
  <div class="formsDiv">
    ...
    <div class="centralize-self" style="margin-top: 10px">
          <label>Microcontroller: Esp32</label>
    </div>
      ...
      <div class="radioButtonDiv" v-show="selectedControlMode == 'Open loop'">
        ...
        <input
          ...
          name="voltageRadioBtn"
          id="voltage3v"
          @click="sendOpenLoopVoltage(3.3)"
          :checked="currentOpenLoopVoltage === 3.3"
        />
        <label for="voltage3v">3.3v</label>
      </div>
    ...
    <div class="formsArea" style="margin-top: 27px;">
      <div class="centralize-self" style="margin-top: 10px">
        ...
        <div v-for="pin of 9" :key="pin">
          <label>{{pin}}° </label>
          <select id="pin" v-model="pins[pin-1]" :disabled="disableControlModeSelection">
            <option  v-for="avaliablePin in [0,12,13,14,25,26,27,32,33,34,35] " :key="avaliablePin">{{avaliablePin}}</option>
